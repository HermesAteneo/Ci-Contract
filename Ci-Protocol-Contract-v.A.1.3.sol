...
contract ERC20 is IERC20, Master {
    ...
    //////////////////////////////////////////// 
    constructor () public {
        //_name           = "Continuous index protocol";
        //_symbol         = "CIN";
        _name           = "Test.A.1.3";
        _symbol         = "TA13";       
        _totalSupply    = 0;
...
contract CiProtocol is ERC20, CiStatistics{
    ...
    //
    // Continuousindex.org is IPFS hosted but the centralized domain could disappear, be hacked or censored by a government but 
    ...
    event PayedToSeller_event(address indexed Seller, uint CiPayed, uint CiPrice, uint Amount);
    function PayToSeller(address payable Seller, uint CiPayed, uint CiPrice) internal proctored {
        ...
        emit PayedToSeller_event(Seller, CiPayed, CiPrice, amountToSeller);
    }
    event PayedToAgent_event(address indexed Buyer, address indexed AgentWallet, uint CiPayed, uint CiPrice, uint Amount);
    function PayToAgent(address Buyer, address payable AgentWallet, uint CiPayed, uint CiPrice) internal proctored {
        ...
        if(AgentWallet != 0x0000000000000000000000000000000000000000 ){
            ...
            require(sentToAgent);
            emit PayedToAgent_event(Buyer, AgentWallet, CiPayed, CiPrice, amountToAgent);
        }
    }
    
    event PayForCi_event(address indexed Buyer, address indexed AgentWallet, uint256 Value, uint64 Native);
    ...
    function PayForCi(address payable AgentWallet) external payable{ 
        ...
        emit PayForCi_event(msg.sender, AgentWallet, msg.value, 0);
    } 
    function () external payable { 
        require( msg.value >= minETHToGet, "Not enough transfer amount" ); 
        if(mintStopped){ require( getTotalAllowanced(-1) != 0, "No Ci for sale now" ); }
        emit PayForCi_event(msg.sender, 0x0000000000000000000000000000000000000000, msg.value, 1);
    }
    ...
    event MigrateFromChain_event(uint CiAmount, uint ChainId, address indexed Owner, uint Allowance, uint AllowanceDate, address AgentWallet);
    function MigrateFromChain(uint CiAmount, uint chainId) external payable{ 
        burn( CiAmount ); //only msg.sender can burn
        emit MigrateFromChain_event(
            CiAmount, 
            chainId,
            msg.sender,
            AccountsAll[msg.sender].Allowance,
            AccountsAll[msg.sender].AllowanceDate,
            AccountsAll[msg.sender].AgentWallet);
    }
    event MigrateToChain_event(uint CiAmount, uint ChainId, bytes32 indexed MigrationHash, address indexed Owner, uint Allowance, uint AllowanceDate, address AgentWallet);
    function MigrateToChain(uint CiAmount, uint chainId, bytes32 migrationHash, address owner, uint allowance, uint allowanceDate, address payable AgentWallet) external proctored { 
        mint( owner, CiAmount, allowance, allowanceDate, AgentWallet );
        emit MigrateToChain_event(CiAmount, chainId, migrationHash, owner, allowance, allowanceDate, AgentWallet);
    }
